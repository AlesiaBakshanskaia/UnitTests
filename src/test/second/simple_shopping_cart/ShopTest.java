...
//        ByteArrayOutputStream output = new ByteArrayOutputStream();
        // Act (Выполнение)
        cart.addProductToCartByID(6); // 170 +
        cart.addProductToCartByID(6); // 170 +
        cart.addProductToCartByID(6); // 170 +
        cart.addProductToCartByID(6); // 170 +
        cart.addProductToCartByID(6); // 170 +
...
//        assertThat(output.toString()).isEqualTo("Этого товара нет в наличии");

    }

    @Disabled
    @Test
    void lastProductsDisappearFromStore2() {
        Shop shop = new Shop(getStoreItems());
        Cart cart = new Cart(shop);
        // Act (Выполнение)
        cart.addProductToCartByID(1);
    ...
    void incorrectProductRemoveCausesException() {
        Shop shop = new Shop(getStoreItems());
        Cart cart = new Cart(shop);
        assertThatThrownBy(() -> cart.removeProductByID(0))
                .isInstanceOf(RuntimeException.class).hasMessage("В корзине не найден продукт с id: 0");
    }
